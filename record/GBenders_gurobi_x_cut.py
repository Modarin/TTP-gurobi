    ...
    for k in range(num_K):
        for s in set_S_over:
            for service in candidate_service[str(k)+'_'+str(s)]:
                ...
                ss=service[1]
                for t in range(len(candidate_T[str(k) + '_' + str(s)])):
                    RP_x.getConstrByName('51_' + str(k) + '_' + str(s) + '_' + str(t) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][0].x)
                    RP_x.getConstrByName('62_' + str(k) + '_' + str(s) + '_' + str(t) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][1].x)
                    RP_x.getConstrByName('71_' + str(k) + '_' + str(s) + '_' + str(t) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][2].x)
                    RP_x.getConstrByName('82_' + str(k) + '_' + str(s) + '_' + str(t) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][3].x)
                for tt in range(len(candidate_T[str(kk) + '_' + str(ss)])):
                    RP_x.getConstrByName('52_' + str(k) + '_' + str(s) + '_' + str(tt) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][0].x)
                    RP_x.getConstrByName('61_' + str(k) + '_' + str(s) + '_' + str(tt) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][1].x)
                    RP_x.getConstrByName('72_' + str(k) + '_' + str(s) + '_' + str(tt) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][2].x)
                    RP_x.getConstrByName('81_' + str(k) + '_' + str(s) + '_' + str(tt) + '_' + str(kk) + '_' + str(ss)).rhs = int(1-lamda[k,s,kk,ss][3].x)
        ...
        for k in range(num_K):
            for s in set_S_over:
                for service in candidate_service[str(k)+'_'+str(s)]:
                    ...
                    ss=service[1]
                    for t in range(len(candidate_T[str(k) + '_' + str(s)])):
                        matrix_chi[k][s][t]+=(lamda[k,s,kk,ss][0].x-lamda[k,s,kk,ss][1].x)*candidate_T[str(k) + '_' + str(s)][t]
                        if t+1 in range(len(candidate_T[str(k) + '_' + str(s)])):
                            matrix_chi[k][s][t+1]-=(lamda[k,s,kk,ss][0].x-lamda[k,s,kk,ss][1].x)*candidate_T[str(k) + '_' + str(s)][t]
                    for tt in range(len(candidate_T[str(kk) + '_' + str(ss)])):
                        matrix_chi[kk][ss][tt]+=(lamda[k,s,kk,ss][1].x-lamda[k,s,kk,ss][0].x)*candidate_T[str(kk) + '_' + str(ss)][tt]
                        if tt+1 in range(len(candidate_T[str(kk) + '_' + str(ss)])):
                            matrix_chi[kk][ss][tt+1]-=(lamda[k,s,kk,ss][1].x-lamda[k,s,kk,ss][0].x)*candidate_T[str(kk) + '_' + str(ss)][tt]
                    obj_para+=(lamda[k,s,kk,ss][1].x + lamda[k,s,kk,ss][3].x) * data.D_br[s]
                    obj_para+=(lamda[k,s,kk,ss][1].x + lamda[k,s,kk,ss][2].x) * data.D_tr[ss]
                    obj_para+=(lamda[k,s,kk,ss][1].x-lamda[k,s,kk,ss][0].x) * data.travel[s]
    ...
    for k in range(num_K):
        for s in set_S_over:
            for service in candidate_service[str(k)+'_'+str(s)]:
                ...
                if p==1:
                    for co in range(4):
                        lhs.addTerms(sub[co]-tem[co], lamda[k, s, kk, ss][co])
                        rhs-=tem[co]*lamda_initial[str(k)+'_'+str(s)+'_'+str(kk)+'_'+str(ss)][co]
                else:
                    for co in range(4):
                        lhs.addTerms(sub[co]-tem[co], lamda[k, s, kk, ss][co])
                        rhs-=tem[co]*lamda[k,s,kk,ss][co].x

    lhs.addTerms(-1, z[0])
    RP_lamda.addConstr(lhs>=rhs,name="benders_"+str(p))
    RP_lamda.update()
    RP_lamda.getConstrByName("benders_"+str(p)).Lazy = 1
    ...
    for k in range(num_K):
        for s in set_S_over:
            for service in candidate_service[str(k)+'_'+str(s)]:
                ...
                ss=service[1]
                for co in range(4):
                    if lamda[k,s,kk,ss][co].x>0.5:
                        count_rhs+=1
                        lhs.addTerms(1,lamda[k,s,kk,ss][co])
                        P_lhs.addTerms(1, P_lamda[k, s, kk, ss][co])

    RP_x_lamda.setObjective(P_lhs, GRB.MAXIMIZE)
    RP_x_lamda.optimize()
    RP_lamda.addConstr(lhs <= min(count_rhs-1,math.floor(RP_x_lamda.ObjVal)), name="benders_" + str(p))
    RP_lamda.update()
    RP_lamda.getConstrByName("benders_" + str(p)).Lazy = 1
...
for k in range(num_K):
    for s in set_S_over:
        for service in candidate_service[str(k)+'_'+str(s)]:
            ...
            lhs0.addTerms(1, lamda[k, s, kk, ss][0])
            lhs1.addTerms(1, lamda[k, s, kk, ss][1])
            lhs2.addTerms(1, lamda[k, s, kk, ss][2])
            lhs3.addTerms(1, lamda[k, s, kk, ss][3])
            lhs.addTerms(min(data.D_br[s],data.D_tr[ss])-1, lamda[k, s, kk, ss][0])
            lhs.addTerms(min(data.D_br[s],data.D_tr[ss])-1, lamda[k, s, kk, ss][1])
            lhs.addTerms(min(data.D_br[s],data.D_tr[ss]), lamda[k, s, kk, ss][2])
            lhs.addTerms(min(data.D_br[s],data.D_tr[ss]), lamda[k, s, kk, ss][3])
...
#     for c in RP_x_lamda.getConstrs():
#         if c.IISConstr:
#             print('%s' % c.constrName)
#             # print(E_tr_x[3][38])
max_lamda[0]=math.floor(RP_x_lamda.ObjVal)
RP_lamda.addConstr(lhs0<=max(0,math.floor(max_lamda[0])), name='max_lamda_'+str(0))
...
RP_lamda.addConstr(lhs1<=max(0,math.floor(max_lamda[1])), name='max_lamda_'+str(1))

RP_x_lamda.setObjective(P_lhs2,GRB.MAXIMIZE)
RP_x_lamda.optimize()
max_lamda[2]=math.floor(RP_x_lamda.ObjVal)
max_lamda[2]=RP_x_lamda.ObjVal
RP_lamda.addConstr(lhs2<=max(0,math.floor(max_lamda[2])), name='max_lamda_'+str(2))
...
RP_lamda.addConstr(lhs3<=max(0,math.floor(max_lamda[3])), name='max_lamda_'+str(3))
...
RP_lamda.addConstr(lhs0+lhs1+lhs2+lhs3<=math.floor(max_lamda[4]), name='max_lamda_'+str(4))
...
RP_lamda.addConstr(z[0]<=lhs, name='max_obj_')
...
while abs((UB[p] - LB[p])/UB[p])>=epsilon:
    ...
    # 更新上界
    UB.append(min(UB[p-1],RP_lamda.ObjVal))
    # 更新下界
    if RP_x.status == GRB.OPTIMAL:
